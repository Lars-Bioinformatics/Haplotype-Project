...
run <- function(mut_name, brca_name){
    ...
    brca2_pheno_merged <- rbind(brca2_pheno[, 1:47], brca2_pheno_hisp) %>% arrange(FamCode)
    ...
    load("cache/brca1_geno_merged.RData")
    load("cache/brca2_geno_merged.RData")
...
## ---- generate_data
generate_geno_data <- function(){
    # Read and merge genotype data - then save R object
    brca1_geno <- read.table("input/139_ouh_june_2017/139_ouh_brca1_onco_geno.txt", header = T)
    brca2_geno <- read.table("input/139_ouh_june_2017/139_ouh_brca2_onco_geno.txt", header = T)
    brca1_geno_hisp <- read.table("input/139_ouh_june_2017/139_ouh_female_hisp_brca1_onco_geno.txt", header = T)
    brca2_geno_hisp <- read.table("input/139_ouh_june_2017/139_ouh_female_hisp_brca2_onco_geno.txt", header = T)
...
plot_haplotype <- function(chr_pos, brca_chr, brca_name, brca_start, brca_stop, mut){
    ...
    p = chr_pos %>% mutate(Country = fct_lump(Country, prop = 0.05)) %>% # group_by(Country) %>%
        ...
        # Print the whole data area - even some snps have been removed.
...
plot_nearest_break <- function(chr_pos, brca_chr, brca_name, brca_start, brca_stop, mut){
    
    chr_pos_positive <- chr_pos %>% filter(position_b37_adjusted > 0) %>% group_by(sample_id) %>% filter(position_b37_adjusted == min(position_b37_adjusted)) #%>% summarise(minPos = min(position_b37_adjusted, minPosName = ))
